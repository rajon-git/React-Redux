...
import {coloSelected, deleted, toggled} from '../redux/todos/actions'
...
const Todo = ({todo}) => {
    ...
    const handleColorChnage =(todoId, color)=>{
        dispatch(coloSelected(todoId, color))
    }
...
>
    <div
        class={`rounded-full bg-white border-2 border-gray-400 w-5 h-5 flex flex-shrink-0 justify-center items-center mr-2 ${completed && 'border-green-500 focus-within:border-green-500'}`} 
    >
        <input
            type="checkbox"
            checked={completed}
